...
### 前提条件
今回作るアプリではマイクを使って録音し、そのデータを音声認識APIに渡します。デバイスやOSのバージョン、使用するアプリの種類によって録音データ音声フォーマットや周波数が異なるため、検証環境を以下に示します。異なる環境では後述する音声フォーマットと周波数の変更が必要になる場合があります。
...
※ なお、ピュアAndroidにはプリインストールされている録音アプリがないため、以下のアプリをインストールしています。

[音声レコーダー](https://play.google.com/store/apps/details?id=com.sonymobile.androidapp.audiorecorder&hl=ja)

この他のアプリでは、今回作成するアプリからの呼び出しができない可能性があります。
