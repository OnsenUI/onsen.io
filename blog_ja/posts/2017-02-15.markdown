...
本日、JINS MEME SDK for Monacaがリリースされました。JINS MEMEとは、眼鏡ブランドのJINSが提供している眼鏡型のウェアラブルデバイスです。健康状態を管理することを目的として、体の傾きやまばたきの状態を計測する機能が搭載されています。
...
`cordova.plugins.JinsMemePlugin.setAppClientID()` はアプリIDとアプリSecretを元にアプリの認証を行う処理です。
第三引数には認証完了後に行う処理、第四引数には認証失敗時に処理を指定します。
...
ただし、アプリの前回起動時にJINS MEMEデバイスとの接続を既に実行している場合は、
`cordova.plugins.JinsMemePlugin.startScan()` を実行する前に切断処理を実行しないとエラーが発生する場合があります。
そこで restartScan() という関数内で計測の停止、デバイスとの切断、スキャンの停止をあらかじめ行っています。
...
デバイス選択ダイアログでデバイスが選択されたら、`cordova.plugins.JinsMemePlugin.connect()` でペアリングを実行します。
第一引数にペアリング完了時の処理、第二引数にデバイスが切断されたときの処理（デバイスの電源が切れた場合やアプリとデバイスが物理的に遠くに離れた場合など）、第三引数にペアリング失敗時の処理を指定します。
...
今回作成したアプリは、単純に計測したデータを表示するだけのサンプルでしたが、視線の動きでページ遷移させるといった機能を盛り込むことも可能です。アイデア次第でまったく新しいアプリを産み出すことができるでしょう。
