...
This sample app is based on an open-source [YouTube Jukebox](http://jgthms.com/juketube/) made by [J. Thomas](http://jgthms.com) on Github. However, here the app has been simplified for beginners, but if you want to see something a little bit more complex (like playlists) you can check out his project!
...
### Consuming the API from an Angular JS controller
...
The way to make a search request to YouTube API v3 is through HTTP Get to `https://www.googleapis.com/youtube/v3/search`. Here we need to specify some values for the query:
* **key**: The API key we got on previous steps.
* **type**: The type of the resource we are asking for (video, playlist, channel).
* **maxResults**: Number of videos that YouTube will return at once.
...
Like these events, there are many other features in YouTube API v3 that you can discover in their docs:
