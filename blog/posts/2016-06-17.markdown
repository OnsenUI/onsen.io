...
title: "It's all about time: Building a performant Stopwatch with MobX and React &mdash; fast"
category: tutorial
...
In [recent posts](https://onsen.io/blog/react-redux-devtools-with-time-travel/) we spoke a lot about developing [React](https://facebook.github.io/react/) applications with [Redux](https://github.com/reactjs/redux): It enables the users to easily make testable and structured applications. However, writing a Redux applications produces a lot of boilerplate code. One framework alternative is [MobX](https://github.com/mobxjs/mobx), which solves this problem by doing automation using observables.
...
Now that we have learned the basics of MobX, lets have a look how to use it with React. First of all, we will need to install the bindings for React:
...
Lets begin by creating a Timer class. The timer class will hold two values: The passed milliseconds since the timer started running and saved milliseconds for the combined time that has passed in previous runnings.
...
Its now time to write the Store class which manages the running state, the start time, the current timer and the saved laps:
...
MobX is great framework for building fast and performant apps. The choice between Redux and MobX comes down to how much control we want to have about the application and how much we let our frameworks do the job. To learn more about MobX we can highly recommend this [video](https://www.youtube.com/watch?v=XGwuM_u7UeQ) and the [GitHub repository](https://github.com/mobxjs/mobx). There is also an [interesting blog](https://medium.com/@mweststrate/3-reasons-why-i-stopped-using-react-setstate-ab73fc67a42e#.r1h8xbd69) post about why it might be a good idea to use the power of MobX to avoid `setState` in React.
