...
As usual, all the code of this example is [available to download on Github](https://github.com/frankdiox/Onsen*ui-router*). You can check here the final result as well before explaining how it is done:
...
In this example we include all the dependencies from CDNs but you can download them locally with [Bower](http://bower.io), for example. We will have two main states for the two tabs called "navigator" and "sliding". The first one will have a child state called "navigator.master" and a grandchild state called "navigator.master.detail". Similarly, "sliding" state will have three childs, one per sub-state: "sliding.main", "sliding.page1" and "sliding.page2". After setting up our `index.html` with the needed dependencies we include the main elements of our app in the body:
...
<ons-template id="html/tab2.html">
	<ons-sliding-menu menu-page="html/menu.html" side="left" var="myMenu" type="reveal" max-slide-distance="260px" swipeable="true">
	</ons-sliding-menu>
</ons-template>
```
...
With `$urlRouterProvider.otherwise("/master");` we are establishing a default route for our app, so when we try to input an inexistent URL/state, the app will show the MasterDetail example. This also works as the index/landing page since we will not define any state for the route `/`.
...
Apart from that, we have to be carefull if we need an `<ons-back-button>` in our pages since we cannot change its inner behavior. Instead we can just use the back button style manually and point it to the state we want to move with `ui-sref`. Usually it will be `ui-sref="^"` since we want to move to the parent state:

```html
<span class="toolbar-button--quiet" style="height: 44px; line-height: 0; padding: 0 10px 0 0; position: relative;" ui-sref="^">
	<i class="ion-ios-arrow-back ons-back-button__icon" style="vertical-align: top; background-color: transparent; height: 44px; line-height: 44px; font-size: 36px; margin-left: 8px; margin-right: 2px; width: 16px; display: inline-block; padding-top: 1px;" data-animit-orig-style="vertical-align: top; background-color: transparent; height: 44px; line-height: 44px; font-size: 36px; margin-left: 8px; margin-right: 2px; width: 16px; display: inline-block; padding-top: 1px;"></i>
	<span style="vertical-align: top; display: inline-block; line-height: 44px; height: 44px;" class="back-button__label" data-animit-orig-style="vertical-align: top; display: inline-block; line-height: 44px; height: 44px;">Back</span>
</span>
```
...
The first "sliding" state is similar to "navigator" state. It simply set up the SlidingMenu as an abstract state. This time "main", "page1" and "page2" will be all direct childs of the Sliding Menu, so we will be able to move between them as siblings. Now we just need to map every `ui-sref` to a `myMenu.setMainPage( ... )`, and we use `onEnter` property again for this purpose. It would be also possible to have a "dynamic state" that receives the page name as a parameter and moves to it with `setMainPage( 'page' )`, so we don't need a new state for every simple page that we have.
