...
author: andreas
date: 2015-07-30
id: nvd3-charts-hybrid-app
title: "Integrate charts in your AngularJS Hybrid Apps with NVD3"
tags: tutorial, angularjs, onsen ui
---

![Monaca-YouTube](/blog/content/images/2015/Jul/d3.png)

There are many options out there if you want to display charts in an hybrid apps. Since a hybrid app is basically a web application running HTML, JavaScript and CSS, you can use any JavaScript charting library or you can even generate charts in a backend and display them as static images.
...
We've recently been discussing if we should support a charting library in Onsen UI, and if so which one we should choose. One of the candidates is NVD3 since it provides a good combination of ease-of-use and the power of D3. It's also very simple to integrate in AngularJS apps, as we will see in this article.
...
First create an empty html element and load the following files:
...
```html
  <html>
    ...
    <body>
      <!-- D3 will use this "svg" tag to draw the chart -->
      <svg></svg>
    </body>
  </html>
```
...
At this point we have not written any code for the directive so only the input elements will work. Implementing the directive is pretty straight forward and we'll borrow some code from the previous pure JavaScript example. I've used two services (`d3` and `nv`) that just return the global objects.
  ...
  .directive('lineChart', ['d3', 'nv', function(d3, nv) {
    return {
      ...
      link: function(scope, element) {
        ...
        // I serializie the data in order to easily check if the data has changed.
...
If you have any questions about the content of the article please write a comment below. In the case you found some mistake or you would like to improve the content, feel free to make a pull request to the [website repository](https://github.com/OnsenUI/onsen.io/tree/master/blog/posts). I appreciate all kind of feedback!
