...
An important thing to note when talking about hybrid apps is that even though the developer might regard it as nothing but a web app, from a user's perspective the app is actually more often compared to native apps since that is what the user is normally running on his or her device. In this case the user's perspective more important. You should not be able to distinguish and ideal hybrid app from it's native equivalent. Of course, it might be impossible to get the exact same performance from an hybrid app, but for the visual appearance it is possible to get very close. That is what we're trying to do with Onsen UI and by adding Material Design we are getting one step closer to that ideal.
...
In order to populate the toolbar we can add some buttons with icons. In order to do that we will use the `<ons-toolbar-button>` and the `<ons-icon>` components.
...
Finally we can add a floating action button to one of the tabs. In the Material Design spec it says that the floating action button should represent the primary action of the app. In our app maybe it can be used to add some data so we will add an icon inside it with a plus. The floating action button is represented by the `<ons-fab>` element in Onsen UI. 
...
Let's go ahead and add a list with some form elements to create a settings page in the left tab. I just copied one of the examples from the Material Design spec so the content is a bit strange. For the full code please take a look at [this gist](https://gist.github.com/argelius/5949819bd89fbfaeba6e).
...
We are continuously developing Onsen UI to make it the best hybrid app framework we can possibly make. Since launching our [Community forum](https://community.onsen.io/) we have received lots of great feedback from the users.
...
If you are having trouble using the beta or if you found a bug, please open an [issue](https://github.com/OnsenUI/OnsenUI/issues)
