...
The following example shows the React extension for Onsen UI in action. It showcases some of components as well as the automatic styling. Please try changing the platform on the first screen to see how it renders on both iOS and Android.
...
The code is pretty self-explanatory. When the component is first created the `constructor` method is run to set the initial state. We set the click counter to 0 in the constructor. The component is rendered using the `render` method which will be run every time the state changes.
...
<iframe height="100" src="http://argelius.github.io/react-sample/"></iframe>
