...
## Step 4: File Components and Explanation
...
Replace the following code to `home.html` file:
...
There is also one controller called `HomeCtrl` which consists of two main functions such as `Login()` and `Logout()`. Inside the `Login()` function, `TwitterConnect.login()` is called asking the user the login with a valid Twitter account information. If you have logged in with a Twitter application on your device, the information of that account will be shown automatically. See below screenshot as example: 

![Authentication](/blog/content/images/2017/Apr/twitter_authentication.png)

After a successful login, `StorageService` is called to store the login information and you will be directed back to `home.html` page showing the profile information of the logged in user. Inside the `Login()` function, a confirmation dialog is shown to confirm the activity (see the screenshot below). If the user select `Yes`, the `TwitterConnect.logout()` function is called and `StorageService` is also called to remove the login information.
